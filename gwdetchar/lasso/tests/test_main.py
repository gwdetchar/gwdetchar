...
expected_ts = io.datafind.get_data(test_channel, test_start,
                                   test_end, frametype=test_frametype)
...
def test_read():
    ...
    try:
        actual_ts = lasso.get_primary_ts(filepath=temp, channel=test_channel,
                                         ...
                                         frametype=test_frametype, cache=None,
                                         nproc=1)
    finally:
        temp.close
    ...
    nptest.assert_array_equal(actual_ts.value, expected_ts.value,
                              err_msg='read in data array does not match')


def test_get():
    print('Testing primary channel query...')
    actual_get_ts = lasso.get_primary_ts(channel=test_channel,
                                         start=test_start, end=test_end,
                                         frametype=test_frametype)
    ...
    nptest.assert_array_equal(actual_get_ts.value,
                              expected_ts.value,
                              err_msg='fetched data array does not match')
