...
AUX['L1:SUS-ITMX_R0_DAMP_L_IN1_DQ'] += 1.064 * PHASE / 2
...
def test_main(data, caplog, tmpdir):
    ...
    simple.main(args)
    assert caplog.text.count("Skipping L1:SUS-BS_M1_DAMP_L_IN1_DQ") == 1
    assert (caplog.text.count("No significant evidence of scattering found")
            == len(simple.MOTION_CHANNELS) - 2)
    assert caplog.text.count(
        "Plotting spectra and projected fringe frequencies") == 1
    assert caplog.text.count("1 chanels plotted") == 1
    # test output
    assert_equal(
        set(os.listdir(outdir)),
        {"L1-SUS_ITMX_R0_DAMP_L_IN1_DQ-34.0-60-comparison.png",
         "L1-SUS_ITMX_R0_DAMP_L_IN1_DQ-34.0-60-overlay.png"},
    )
    # clean up
